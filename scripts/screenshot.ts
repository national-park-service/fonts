...
function parseArgs(): Args {
  ...
  const target = positional[0] ?? 'web/review/index.html'
  return {
    htmlPath: /^https?:\/\//i.test(target) ? target : resolve(root, target),
...
async function main(): Promise<void> {
  ...
  const isUrl = /^https?:\/\//i.test(args.htmlPath)
  let dataUrl: string
  if (isUrl) {
    dataUrl = args.htmlPath
  }
  else {
